...
class IQ(nn.Module):
    ...
    def __init__(self, latent_transformer, image_only, vocab, args, num_att_layers=2):
        ...
        self.answer_encoder = GVTransformerEncoder(
            self.embedding, self.latent_layer, self.latent_transformer, args)
    ...
    # def switch_image_mode(self, new_mode):
    #     self.image_only = new_mode
    ...
    def embedder(self):
        ...
        if self.args.emb_file is not None:
            ...
            pre_trained = 0
            for line in open(os.path.join(self.args.root_dir, self.args.emb_file), encoding="utf8").readlines():
                sp = line.split()
    ...
    def forward(self, images, textual_input, response, target):
        ...
        if self.args.variant == "image-only":
            kld_loss = None
            image_pad_mask = torch.zeros(images.shape[0],
                                         1,
                                         1,
                                         device=self.args.device).bool()
            output, target_embedding, z_logit = self.decoder(image_features.unsqueeze(1),
                                                             target,
                                                             image_features,
                                                             image_features,
                                                             image_pad_mask)
        ...
        if self.args.variant == "text-only":
            encoder_outputs, kld_loss, z, transformer_posteriors, src_mask = self.answer_encoder(
                context=textual_input,
                response=response,
                image_features=image_features  # NEED TO WORK ON THIS
            )
            if self.latent_transformer:
                z = self.latent_projection(z)

            output, target_embedding, z_logit = self.decoder(
        ...
        if self.args.variant == "image-text-without-image-recon":
            encoder_outputs, kld_loss, z, transformer_posteriors, src_mask = self.answer_encoder(
                textual_input, response, image_features)
        ...
        if self.args.variant == "image-text-with-image-recon":
    ...
    def decode_greedy(self, images, answers, max_decode_length=50):
        ...
        if self.args.variant == "image-only":
            decoder_input_mask = torch.zeros(images.shape[0],
                                             1,
                                             1,
                                             device=self.args.device).bool()
            ...
            for i in range(max_decode_length + 1):
                pred_targets_logit = self.decoder.inference_forward(
        ...
        encoder_outputs = self.answer_encoder.encoder(
            embedded_context, src_mask)
        encoder_outputs[:, 0] = encoder_outputs[:, 0] + \
            image_features  # TEST THISSS
        ...
        for i in range(max_decode_length + 1):
            pred_targets_logit = self.decoder.inference_forward(
                encoder_outputs, ys, image_features, z, src_mask)
            _, pred_next_word = torch.max(pred_targets_logit[:, -1], dim=1)
            top_6_vals, top_6_indicies = torch.topk(
                torch.nn.functional.softmax(pred_targets_logit[:, -1], -1), 6, dim=1)
