...
class GVTransformerDecoder(nn.Module):
    ...
    def forward(self, encoder_outputs, target, image_features, z, src_mask):
        ...
        sos_token = torch.LongTensor(
            [self.vocab.word2idx[self.vocab.SYM_SOQ]] * encoder_outputs.size(0)).unsqueeze(1)
        ...
        target_embedding[:, 0] = target_embedding[:, 0] + \
            image_features  # z = 0 if we're pretraining
        z_logit = None
        ...
        decoder_outputs, _ = self.decoder(
            target_embedding, encoder_outputs, (src_mask, trg_key_padding_mask))
    ...
    def inference_forward(self, encoder_outputs, inference_input, image_features, z, src_mask):
        ...
        if self.args.variant in ["image-text-with-image-recon", "image-text-without-image-recon", "image-only"]:
            pred_targets_embedding[:,
                                   0] = pred_targets_embedding[:, 0] + z + image_features
        decoder_outputs, _ = self.decoder(
            pred_targets_embedding, encoder_outputs, (src_mask, trg_key_padding_mask))
