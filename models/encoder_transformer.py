...
class GVTransformerEncoder(nn.Module):
    ...
    def forward(self, context, response, image_features):
        ...
        # concatanating img features with the encoded outputs
        if self.args.variant in ["image-text-with-image-recon", "image-text-without-image-recon"]:
            encoder_outputs[:, 0] = encoder_outputs[:, 0] + image_features
        kld_loss, z, posteriors = None, None, None  # kl divergence, posterior, z
